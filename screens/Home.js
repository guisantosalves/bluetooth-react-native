...
const Home = ({navigation}) => {
  ...
  const saveCSV = () => {
    ...
    // nao pode salvar com o mesmo nome de um arquivo já existente
    const path = RNFS.DownloadDirectoryPath  + `/pesagens${new Date().getTime()}.csv`;

    console.log(path)
    if(dataToDisplayFromAS.length > 0){
      console.log(dataToDisplayFromAS[1])
      // mounting the csv file
      const headerString = `Brinco,Brinco Eletronico,Peso,Idade,Sexo,Raca,Valor,Obeservacao\n`;

      // tem que ser sem o {} pois está retornando e não executando de fato
      const rowString = dataToDisplayFromAS.map((item, index)=>`${item.brinco},${item.brincoEletronico},${item.peso},${item.idade},${item.sexo},${item.raca},${item.valorMedio},${item.observacao}\n`,
      ).join('')
